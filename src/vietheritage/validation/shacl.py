...
def run(run_mode: str = "sample", data_path: Path | None = None) -> int:
    data = data_path or (REPO_ROOT / "data" / "rdf" / "vietheritage.ttl")
    out = REPO_ROOT / "reports" / run_mode
    out.mkdir(parents=True, exist_ok=True)
    if not data.exists() or not SHAPES_PATH.exists():
        report = {"status": "FAIL", "conforms": False, "errors": ["SHACL artifact missing"]}
        (out / "shacl.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
        return 1
    try:
        conforms, results_text, results_graph = validate_graph(data)
        report: dict[str, Any] = {
        ...
        report = {"status": "FAIL", "conforms": False, "errors": [str(exc)]}
    (out / "shacl.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
